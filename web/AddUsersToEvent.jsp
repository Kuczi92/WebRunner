<%-- 
    Document   : AddUsersToEvent
    Created on : 2017-09-07, 23:31:03
    Author     : Quchi
--%>
...
<html>
    ...
    Integer EventID = (Integer) request.getSession().getAttribute("EventID");
           ...
           em.getTransaction() //rozpoczęcie tranzakcji    
            .begin();
                Query query = em.createQuery("select E from Event E where  E.idevent=:EventID").setParameter("EventID", EventID);
                ChosenEvent =  (Event)query.getSingleResult();
                
          Query query2 = em.createQuery("Select U from User U where U != all" + 
          "(select U from User U left outer join U.numberList N where N.numberPK.eventIdevent=:EventID) ")
                   .setParameter("EventID", ChosenEvent.getIdevent());
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="page-header">
                <h1>Dodaj danego użytkownika do biegu.</h1>
            </div>
        </div>
    </div>
    <div class="row">
        <% 
            String ErrorMessege = request.getParameter("error");
            if (ErrorMessege!=null)
            {
                    %>
                     <div class="col-lg-4 col-lg-offset-4">
                       <p class="h6"><% out.print(ErrorMessege.replace("_", " ")); %></p>
                     </div>
                    <%
            }
            %>
                ...
                <% for (User CurrentUser : Users){
                    ...
                    %>
                           ...
                           <tr>
                              ...
                              <td>
                                  <form class="form-inline" name="DeleteChoosenPicture"  method="post" action="AddUserToSelectedEvent">
                                       <div class="form-group row">
                                          <div class="col-md-8 col-sm-8">
                                          <input name="inputNewNumber" type="text" class="form-control" placeholder="Wpisz numer startowy" required />
                                          </div>
                                          <div class="col-md-4 col-sm-8">
                                          <button class="btn btn-primary btn-md" type="submit" name="ChoosenUserNameAdd" value="<%out.print(CurrentUser.getUsername());%>" >Dodaj</button>
                                          </div>
                                        </div>
                                  </form>  
                              </td>
